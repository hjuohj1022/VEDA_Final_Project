...
QString Backend::sunapiExportExtractKvValue(const QString &text, const QStringList &keys) const {
    // Key=Value 본문에서 첫 번째 매칭 키의 값을 추출
...
QString Backend::sunapiExportExtractJsonString(const QJsonObject &obj, const QStringList &keys) const {
    // JSON 타입이 문자열/숫자/불리언이어도 문자열로 통일 반환
    for (const QString &k : keys) {
        if (obj.contains(k)) {
...
bool Backend::sunapiExportParseCreateReply(const QByteArray &body,
    ...
    if (pe.error == QJsonParseError::NoError && doc.isObject()) {
        ...
        if (jobId) *jobId = sunapiExportExtractJsonString(obj, {"JobID", "JobId", "ExportID", "id"});
        if (downloadUrl) *downloadUrl = sunapiExportExtractJsonString(obj, {"DownloadUrl", "DownloadURL", "Url", "url", "FileUrl"});
        if ((jobId && !jobId->isEmpty()) || (downloadUrl && !downloadUrl->isEmpty())) return true;
    }

    const QString text = QString::fromUtf8(trimmed);
    // JSON 실패 시 Key=Value 텍스트 응답으로 폴백
    if (jobId) *jobId = sunapiExportExtractKvValue(text, {"JobID", "JobId", "ExportID", "id"});
    if (downloadUrl) *downloadUrl = sunapiExportExtractKvValue(text, {"DownloadUrl", "DownloadURL", "Url", "url", "FileUrl"});

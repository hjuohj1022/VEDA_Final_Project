...
void BackendSunapiExportWsSessionService::requestPlaybackExportViaWs(Backend *backend, BackendPrivate *state, int channelIndex,
    ...
    if (startSec < 0 || endSec < 0 || endSec < startSec) {
        state->m_playbackExportInProgress = false;
        emit backend->playbackExportFailed("내보내기 실패: 시작/종료 시간 형식 오류");
        return;
    }
    ...
    QString prepError;
    if (!backend->buildPlaybackExportWsOutputPath(savePath, &wantsAvi, &outPath, &finalOutPath, &prepError)) {
        state->m_playbackExportInProgress = false;
        emit backend->playbackExportFailed(prepError);
        return;
    }
    ...
    backend->attachIgnoreSslErrors(sessionReply, "SUNAPI_EXPORT_WS_SESSION");

    QObject::connect(sessionReply, &QNetworkReply::finished, backend, [=]() {
        if (sessionReply->error() != QNetworkReply::NoError) {
            const QString e = sessionReply->errorString();
            sessionReply->deleteLater();
            state->m_playbackExportInProgress = false;
            emit backend->playbackExportFailed(QString("내보내기 실패: 세션 요청 실패 (%1)").arg(e));
            return;
        }
        ...
        if (rtspUri.isEmpty()) {
            state->m_playbackExportInProgress = false;
            emit backend->playbackExportFailed("내보내기 실패: RTSP URI 누락");
            return;
        }
        if (realm.isEmpty() || nonce.isEmpty()) {
            state->m_playbackExportInProgress = false;
            emit backend->playbackExportFailed("내보내기 실패: digest challenge 누락");
            return;
        }
        if (digestResponse.isEmpty()) {
            state->m_playbackExportInProgress = false;
            emit backend->playbackExportFailed("내보내기 실패: digest response 누락");
            return;
        }
        if (wsUser.isEmpty()) {
            state->m_playbackExportInProgress = false;
            emit backend->playbackExportFailed("내보내기 실패: digest 사용자 정보 누락");
            return;
        }

        emit backend->playbackExportProgress(3, "내보내기 세션 연결 준비");
        ...
        if (!st->outFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            emit backend->playbackExportFailed(QString("내보내기 실패: 파일 열기 실패 (%1)").arg(st->outFile.errorString()));
            return;
        }

        auto sendRtsp = [backend, st](const QByteArray &rtspText) {
        ...
        auto finishWith = [backend, state, st](bool ok, const QString &message) {
            if (st->finished) {
                return;
            }
            st->finished = true;
            ...
            if (ok) {
                if (st->needsAviRemux) {
                    ...
                    emit backend->playbackExportProgress(99, "AVI 변환 중");
                    const QString ffmpegBin = backend->resolvePlaybackExportFfmpegBinary();
                    ...
                    if (!started || !done || ff.exitStatus() != QProcess::NormalExit || ff.exitCode() != 0) {
                        ...
                        emit backend->playbackExportFailed(QString("내보내기 실패: AVI 변환 실패 (%1)")
                ...
                emit backend->playbackExportFinished(st->needsAviRemux ? st->finalOutPath : st->outPath);
                ...
                state->m_playbackExportOutPath.clear();
                state->m_playbackExportFinalPath.clear();
                emit backend->playbackExportFailed(message);
            }
        };

        auto parseRtspResponse = [backend, state, st, sendRtsp, finishWith](const QString &text) {
            if (state->m_playbackExportCancelRequested) {
                finishWith(false, "내보내기 취소됨");
                return;
            }
            backend->playbackExportHandleRtspResponse(
                text,
                st->setupCseqTrack,
                st->setupDoneCount,
                st->trackInterleaved,
        ...
        auto processInterleaved = [backend, state, st, finishWith](const QByteArray &bytes) {
            if (state->m_playbackExportCancelRequested) {
                finishWith(false, "내보내기 취소됨");
                return;
            }
            if (backend->playbackExportConsumeInterleaved(bytes,
                                                 st->h264RtpChannel,
                                                 st->interleavedBuf,
                                                 st->outFile,
                                                 st->writtenBytes,
        ...
        st->ws = new QWebSocket(QString(), QWebSocketProtocol::VersionLatest, backend);
        ...
        QObject::connect(st->hardTimeoutTimer, &QTimer::timeout, backend, [backend, state, finishWith, st]() {
            if (state->m_playbackExportCancelRequested) {
                finishWith(false, "내보내기 취소됨");
                return;
            }
        ...
        QObject::connect(st->keepAliveTimer, &QTimer::timeout, backend, [backend, st, sendRtsp, finishWith, durationSec]() {
            ...
            QByteArray req = backend->buildPlaybackExportRtspRequest(st->nextCseq,
                                                            st->authHeader,
                                                            st->session,
                                                            "GET_PARAMETER",
                                                            st->uri.toUtf8(),
        ...
        QObject::connect(st->ws, &QWebSocket::connected, backend, [backend, st, sendRtsp]() {
            // OPTIONS 2회 + track별 SETUP 요청 전송
            QByteArray options1;
            ...
            QByteArray options2 = backend->buildPlaybackExportRtspRequest(st->nextCseq,
                                                                 st->authHeader,
                                                                 st->session,
                                                                 "OPTIONS",
                                                                 st->uri.toUtf8(),
            ...
            for (const QString &track : tracks) {
                ...
                QByteArray setup = backend->buildPlaybackExportRtspRequest(st->nextCseq,
                                                                  st->authHeader,
                                                                  st->session,
                                                                  "SETUP",
        ...
        QObject::connect(st->ws, &QWebSocket::binaryMessageReceived, backend, [parseRtspResponse, processInterleaved](const QByteArray &payload) {
            // RTSP 제어 응답과 interleaved RTP 데이터 분기 처리
        ...
        QObject::connect(st->ws, &QWebSocket::errorOccurred, backend, [backend, state, finishWith, st](QAbstractSocket::SocketError) {
            if (state->m_playbackExportCancelRequested) {
        ...
        QObject::connect(st->ws, &QWebSocket::disconnected, backend, [backend, state, finishWith, st]() {
            if (state->m_playbackExportCancelRequested) {
                // 취소 중 disconnected는 정상 해제 경로로 처리
                st->finished = true;
                state->m_playbackExportWs = nullptr;
                if (st->outFile.isOpen()) {
                    st->outFile.flush();
        ...
        emit backend->playbackExportProgress(5, "내보내기 WebSocket 연결 시작");
        state->m_playbackExportWs = st->ws;
        ...
        if (wsScheme == "wss" && state->m_sslConfigReady) {
            st->ws->setSslConfiguration(state->m_sslConfig);
            QObject::connect(st->ws, &QWebSocket::sslErrors, backend, [backend, state, st](const QList<QSslError> &errors) {
                for (const auto &err : errors) {
                    qWarning() << "[SUNAPI_EXPORT_WS][SSL]" << err.errorString();
                }
                if (state->m_sslIgnoreErrors && st && st->ws) {
        ...
        QString wsPath = state->m_env.value("SUNAPI_STREAMING_WS_PATH").trimmed();
        if (wsPath.isEmpty()) {

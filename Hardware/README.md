...
* **설명:** 열화상 프레임은 대역폭 사용량이 크고 지속 전송이 필요하며, 서보/레이저 제어는 작은 패킷이지만 지연과 누락에 민감합니다. 본 시스템은 이를 분리하기 위해 `Teensy -> ESP32 -> DTLS/UDP` 경로를 데이터 평면으로, `Client -> MQTT/TLS -> ESP32 -> UART -> STM32` 경로를 제어 평면으로 구성합니다. 이 구조는 네트워크 부하가 큰 프레임 스트리밍이 제어 응답성을 직접 저하시킬 가능성을 줄이고, 보드별 책임을 명확히 하여 디버깅과 확장을 용이하게 합니다.

###### 주요 구성 (Major Components)

| 구성 요소명 | 컴포넌트의 핵심 역할 | 핵심 소스/실행 파일 경로 |
| --- | --- | --- |
| ESP32-C5 Gateway | Wi-Fi 연결, MQTT/TLS 클라이언트, DTLS/UDP 프레임 송신, Teensy SPI 프레임 재조립, STM32 UART 브리지 | `esp32/main/main.c`, `esp32/main/mqtt/`, `esp32/main/device/` |
...
| `esp32/main/mqtt/mqtt.c` | MQTT subscribe/publish 처리, health JSON 생성, 열화상 프레임을 DTLS/UDP 또는 MQTT chunk로 송신하는 정책을 담당합니다. |
| `esp32/main/mqtt/udp_stream.c` | PSK 기반 DTLS client 초기화, handshake, 송신 재시도, 혼잡 상태 판단, Wi-Fi reconnect 이후 재초기화를 담당합니다. |
| `esp32/main/mqtt/wifi.c` | Wi-Fi STA 연결, IP 획득 후 MQTT client 및 DTLS 초기화 타이밍 제어를 담당합니다. |
...
[ Client / Server / Operator Tool ]
        ...
        [ MQTT Broker ]
          ...
          [ ESP32-C5 ]
          |         |
          | UART1   | DTLS/UDP or MQTT frame stream
          v         v
...
* **기능 1** : FLIR Lepton의 160x120 16-bit 열화상 프레임을 Teensy 4.1에서 캡처하고 ESP32-C5를 통해 DTLS/UDP 또는 MQTT chunk로 송신
...
* **기능 5** : SPI magic/checksum/sequence 검증, Lepton capture timeout reset, DTLS 재연결 지연 초기화 등 장애 대응 로직 포함
...
* **필수 변수** :
  * ESP32 애플리케이션 설정: `APP_WIFI_SSID`, `APP_WIFI_PASS`, `APP_MQTT_BROKER_URI`, `APP_DTLS_TARGET_HOST`, `APP_DTLS_TARGET_PORT`, `APP_DTLS_PSK_IDENTITY`, `APP_DTLS_PSK_KEY_HEX`, `APP_FRAME_STREAM_MODE`, `APP_UDP_FRAME_8BIT`
  * ESP-IDF 환경: `IDF_PATH`
...
* `APP_DTLS_TARGET_HOST`: 열화상 DTLS 수신기 호스트 또는 IP
* `APP_DTLS_TARGET_PORT`: DTLS 수신 포트, 기본값 `5005`
* `APP_DTLS_PSK_IDENTITY`: DTLS PSK identity 문자열
* `APP_DTLS_PSK_KEY_HEX`: DTLS PSK hex 문자열, 비어 있으면 DTLS 비활성
* `APP_FRAME_STREAM_MODE`:
  * `0`: MQTT only
  * `1`: DTLS/UDP only
  * `2`: DTLS/UDP + MQTT 동시 송신
  * `3`: DTLS/UDP frame + MQTT control/status
* `APP_UDP_FRAME_8BIT`:
  * `0`: raw 16-bit frame chunk 전송
  * `1`: 8-bit normalized frame chunk 전송
* IP/PORT:
  * MQTT는 `APP_MQTT_BROKER_URI`
  * DTLS는 `APP_DTLS_TARGET_HOST` + `APP_DTLS_TARGET_PORT`
...
* 보안 정책:
  ...
  * DTLS는 certificate 방식이 아니라 PSK 기반 인증을 사용
...
* **Smoke Test** :
  ...
  * DTLS receiver에서 `APP_DTLS_TARGET_PORT` 기본값 `5005`로 frame chunk 유입 확인
...
* **Start/Channel Select** :
  ...
  * Wi-Fi 연결 이후 MQTT client와 DTLS 초기화 타이밍이 결정됩니다.
...
* **Stop** :
  ...
  * ESP32는 Wi-Fi disconnect 시 DTLS reset request를 설정하고 재연결 후 지연 초기화를 수행합니다.
...
| ESP32 -> DTLS/UDP 또는 MQTT frame chunk | DTLS/UDP는 무연결, MQTT는 QoS 0 | `frame_id[2]`, `chunk_idx[2]`, `total_chunks[2]`, `min[2]`, `max[2]` | DTLS/UDP 시 최대 1024바이트, MQTT-only 시 최대 1200바이트 |
...
| 현상 | 원인 | 해결책 |
| --- | --- | --- |
| DTLS frame이 수신되지 않음 | `APP_DTLS_TARGET_HOST`, `APP_DTLS_PSK_KEY_HEX`, 수신 포트 설정 오류 | `app_secrets.h`의 DTLS 설정과 receiver 포트 상태 확인 |
...
| Wi-Fi reconnect 후 프레임 복구 지연 | DTLS가 reconnect 직후 즉시 올라오지 않고 deferred init 수행 | ESP32 로그에서 `DTLS init deferred` 및 handshake 성공 여부 확인 |

###### Operational Checklist (배포 전 최종 확인)

* 설정 파일의 Wi-Fi, MQTT broker, DTLS target, IP/Port가 운영 환경에 맞게 수정되었는가?
* ESP32 인증서 파일 3종이 `esp32/main/certs/` 경로에 배치되었는가?
* DTLS PSK identity/key가 실제 receiver 설정과 일치하는가?
...
* 열화상 소비자 측에서 DTLS 포트 `5005` 또는 선택한 포트가 열려 있는가?

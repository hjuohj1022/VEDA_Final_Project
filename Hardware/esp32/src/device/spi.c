...
esp_err_t spiMasterInit(void)
{
    ...
    if (!spi_cmd_queue) printf("SPI queue create failed\n");
...
{
    if (!data || len == 0 || len > MAX_SPI_DATA_LEN) return ESP_ERR_INVALID_ARG;

    uint8_t tx[SPI_PKT_SIZE] = {0};
    uint8_t rx[SPI_PKT_SIZE] = {0};
...
esp_err_t spiRead(uint8_t *out_buf, uint8_t *out_len)
{
    ...
    *out_len = rx[1];
    if (*out_len > MAX_SPI_DATA_LEN) *out_len = MAX_SPI_DATA_LEN;
...
void spiTask(void *arg)
{
    ...
    while (1) {
        if (xQueueReceive(spi_cmd_queue, &cmd, portMAX_DELAY) == pdTRUE) {
            printf("SPI sending: '%.*s' (%d bytes)\n", cmd.len, cmd.data, cmd.len);
            spiWrite(cmd.data, cmd.len);
        }
    }
}
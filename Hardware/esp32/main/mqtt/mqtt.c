...
#define MQTT_FRAME_MIN_HEAP_BYTES        32768U
...
static bool mqttHasHealthyLink(void)
{
    ...
    return (esp_get_free_heap_size() >= MQTT_FRAME_MIN_HEAP_BYTES);
}

static void mqttDropStaleFrames(void)
{
    for (;;) {
        ...
        (void)printf("Dropping stale frame id=%u to keep latest frame only (queued=%u)\n",
...
void mqttFrameTask(void *arg)
{
    ...
    for (;;) {
        ...
        for (uint16_t i = 0U; i < total_chunks; i++) {
            if ((s_mqtt_connected == false) || !mqttHasHealthyLink()) {
                send_ok = false;
                failed_chunk = (int)i;
                break;
            }

...
Crow Server는 Raspberry Pi K3s 클러스터 내부에서 동작하는 REST/WebSocket 게이트웨이입니다. 사용자 인증과 계정 관리, SUNAPI 프록시, CCTV 제어, 열화상 스트림 제어, 녹화 파일 관리, 이벤트 로그 조회, MQTT 기반 모터/레이저/ESP32 상태 제어를 통합 제공하는 서버 컴포넌트입니다.
...
| Mail Recovery Integration | 이메일 인증/비밀번호 재설정 코드 메일 발송 | `src/AppScriptMailSender.cpp`, `src/AuthRecoveryRoutes.cpp` |
| SUNAPI Proxy | 카메라 SUNAPI HTTP 프록시 | `src/SunapiProxy.cpp` |
| SUNAPI WS Proxy | StreamingServer WebSocket 프록시 | `src/SunapiWsProxy.cpp` |
| Playback / Export Session API | Playback/Export용 RTSP digest 세션 정보 제공 | `src/SunapiProxy.cpp` |
...
| Event Log API | 서버 측 이벤트 이력 조회/삭제 | `src/EventLogStore.cpp`, `src/EventLogRoutes.cpp` |
| Recording File API | 녹화 파일 목록/삭제/Range 스트리밍, 저장소 사용량 조회 | `src/main.cpp` |
...
| `src/AuthRecoveryRoutes.cpp` | 이메일 인증, 비밀번호 재설정, 관리자 잠금 해제 등 계정 복구 흐름 담당 |
| `src/AppScriptMailSender.cpp` | Apps Script 메일 웹앱 호출로 인증/재설정 코드 발송 |
| `src/SunapiProxy.cpp` | `/sunapi/stw-cgi/*`, storage/timeline/display/playback/export 보조 API를 실제 카메라 SUNAPI로 전달 |
...
| `src/EspHealthManager.cpp` | `system/control`, `system/status` 기반 ESP32 watchdog 제어와 최신 상태 조회 |
| `src/EventLogRoutes.cpp` / `src/EventLogStore.cpp` | `/events` API와 MariaDB 기반 이벤트 로그 저장/조회/삭제 처리 |
...
- **기능 1:** JWT 기반 로그인, 회원가입, 이메일 인증, 비밀번호 재설정, 관리자 잠금 해제, 2FA, 계정 삭제/비밀번호 변경 API 제공
- **기능 2:** SUNAPI HTTP/WebSocket 프록시를 통해 storage/timeline/display/playback/export 기능을 중앙 API로 노출
- **기능 3:** CCTV backend 제어와 WebSocket 스트림을 프록시하여 외부 클라이언트와 연결
- **기능 4:** 내부 UDP 열화상 패킷을 WebSocket 스트림으로 브리지하고 서버 측 이벤트 감지/자동 제어를 수행
- **기능 5:** MQTT를 통해 모터, 레이저, ESP32 watchdog 명령을 제어하고 최신 상태를 조회
- **기능 6:** `/events`, `/recordings`, `/stream`, `/system/storage`를 통해 이벤트 이력과 녹화 자산을 관리
- **기능 7:** `/health`, `/docs`, `/swagger.yaml`를 통해 운영 점검과 문서 접근 지원
...
- **필수 환경 변수:**
  ...
  - `JWT_SECRET` (운영 환경에서는 기본값 대신 반드시 지정 권장)
  - `ADMIN_UNLOCK_KEY` (`crow-admin-secret` Secret에서 주입)
  - `MAIL_APP_SCRIPT_URL`, `MAIL_SHARED_SECRET`
...
- **선택 환경 변수 또는 기본값 제공 변수:**
  - `MAIL_APP_SCRIPT_TIMEOUT_MS`
  - `MQTT_HOST`, `MQTT_PORT`
  - `MOTOR_MQTT_CLIENT_ID`, `MOTOR_CONTROL_TOPIC`, `MOTOR_RESPONSE_TOPIC`
  - `MOTOR_COMMAND_TIMEOUT_MS`
  - `MOTOR_EMERGENCY_SCRIPT` (`;` separated emergency evacuation motor sequence)
  - `LASER_CONTROL_TOPIC`
  - `ESP32_WATCHDOG_CLIENT_ID`, `ESP32_SYSTEM_CONTROL_TOPIC`, `ESP32_SYSTEM_STATUS_TOPIC`
  - `SUNAPI_RTSP_HOST`, `SUNAPI_STREAMING_WS_PATH`
  - `SUNAPI_EXPORT_CREATE_CGI`, `SUNAPI_EXPORT_CREATE_SUBMENU`, `SUNAPI_EXPORT_CREATE_ACTION`, `SUNAPI_EXPORT_CREATE_QUERY`
  - `SUNAPI_EXPORT_POLL_CGI`, `SUNAPI_EXPORT_POLL_SUBMENU`, `SUNAPI_EXPORT_POLL_ACTION`, `SUNAPI_EXPORT_POLL_QUERY`
  - `SUNAPI_EXPORT_DOWNLOAD_CGI`, `SUNAPI_EXPORT_DOWNLOAD_SUBMENU`, `SUNAPI_EXPORT_DOWNLOAD_ACTION`, `SUNAPI_EXPORT_DOWNLOAD_QUERY`
  - `AUTH_DEBUG_TOKEN_RESPONSE` (디버그 토큰 응답 노출용, 개발 전용)
- **경로 주의사항:**
...
- **Secret/ConfigMap 주의사항:**
  ...
  - 이메일 인증/비밀번호 재설정 메일 발송은 `crow-mail-secret`의 `MAIL_APP_SCRIPT_URL`, `MAIL_SHARED_SECRET`, `MAIL_APP_SCRIPT_TIMEOUT_MS`를 사용합니다.
  - Telegram 알림 연동은 `crow-telegram-secret`의 `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`를 사용합니다.
  - SUNAPI 카메라 계정은 `crow-sunapi-secret`으로 관리합니다.
  - 계정 복구/이벤트 로그 기능은 MariaDB에 `signup_email_verifications`, `password_reset_tokens`, `event_logs` 테이블이 준비되어 있어야 합니다.
...
- `THERMAL_ENABLE_FRAME_STATS`, `THERMAL_UDP_RCVBUF_BYTES`, `THERMAL_FRAME_TRACK_TIMEOUT_MS`, `THERMAL_MAX_TRACKED_FRAMES`, `THERMAL_STATS_LOG_INTERVAL_MS`: UDP 수신 버퍼와 frame tracker/stats 로그 동작을 조절합니다.
...
###### 설정 파일명 2: `crow-sunapi-config.yaml`
...
###### 설정 파일명 3: `crow-cctv-ip-config.yaml`

- `CCTV_BACKEND_HOST`
- `CCTV_BACKEND_PORT`

###### 설정 파일명 4: `crow-sunapi-secret.yaml`

- `SUNAPI_USER`
- `SUNAPI_PASSWORD`

###### 설정 파일명 5: `crow-db-migration-job.yaml` / `2fa_migration.sql`

- `users` 테이블의 2FA 관련 컬럼 보강용 Job 예시
- Job 실행 전 `2fa_migration.sql`로 `crow-db-migration-sql` ConfigMap을 만들어야 합니다.
- 현재 저장소의 `2fa_migration.sql`은 `event_logs` 테이블을 생성하지 않으므로, `/events` API를 사용할 경우 MariaDB에 별도 DDL을 적용해야 합니다.
- `event_logs` 권장 스키마는 `mariadb/README.md`의 테이블 예시를 따릅니다.
...
- **Smoke Test:**
  ...
  - `GET /laser/status` 확인
  - `GET /events?limit=10` 확인
  - `GET /recordings`, `GET /system/storage` 확인
  - `GET /esp32/watchdog/status` 확인
...
- **Start/Channel Select:**
  ...
  - `main.cpp`에서 `registerSunapiProxyRoutes`, `registerSunapiWsProxyRoutes`, `registerCctvProxyRoutes`, `registerThermalProxyRoutes`, `registerMotorRoutes`, `registerLaserRoutes`, `registerEspHealthRoutes`, `registerAuthRecoveryRoutes`, `registerEventLogRoutes`와 recordings/storage/auth 라우트를 등록합니다.
- **Pause/Resume:**
...
| 기능 | 메서드 | 설명 |
| --- | --- | --- |
| `/register` | `POST` | 회원가입 및 이메일 인증 완료 여부 검증 |
| `/login` | `POST` | 비밀번호 1차 인증, 2FA 계정은 `pre_auth_token` 반환 |
| `/auth/email/verify/request`, `/auth/email/verify/confirm` | `POST` | 회원가입 전 이메일 인증 코드 발급/확인 |
| `/auth/password/forgot`, `/auth/password/reset` | `POST` | 비밀번호 재설정 코드 발급/적용 |
| `/auth/admin/unlock` | `POST` | 관리자 잠금 해제 키 검증 |
| `/2fa/status` | `GET` | 현재 로그인 사용자 2FA 상태 조회 |
| `/2fa/setup/init`, `/2fa/setup/confirm` | `POST` | TOTP 2FA 등록 시작/확정 |
| `/2fa/verify`, `/2fa/disable` | `POST` | OTP 최종 로그인 / 2FA 비활성화 |
| `/account/password/change`, `/account/delete` | `POST` | 비밀번호 변경 / 계정 삭제 |
| `/events` | `GET`, `DELETE` | 이벤트 로그 조회 및 단건/전체 삭제 |
| `/recordings` | `GET`, `DELETE` | MP4 녹화 파일 목록 조회 및 파일 삭제 |
| `/stream` | `GET` | Range 요청을 지원하는 MP4 스트리밍 |
| `/system/storage` | `GET` | 녹화 저장소 사용량 조회 |
| `/api/sunapi/storage`, `/api/sunapi/timeline`, `/api/sunapi/month-days` | `GET` | 저장소/타임라인/월별 녹화일 조회 |
| `/api/sunapi/ptz/focus` | `POST` | `zoom_*`, `focus_*`, `autofocus` PTZ/Focus 명령 프록시 |
| `/api/sunapi/display/settings`, `/api/sunapi/display/reset` | `GET`, `POST` | 표시 설정 조회/적용/초기화 |
| `/api/sunapi/playback/challenge`, `/api/sunapi/playback/digestauth` | `GET` | RTSP digest challenge/response 생성용 보조 API |
| `/api/sunapi/playback/session`, `/api/sunapi/export/session` | `GET` | RTSP digest + WS 연결용 세션 정보 생성 |
| `/api/sunapi/export/create`, `/api/sunapi/export/status`, `/api/sunapi/export/download` | `GET` | Export 작업 생성/상태/다운로드 프록시 |
| `/sunapi/StreamingServer` | `WS` | 카메라 StreamingServer WebSocket 프록시 |
| `/cctv/control/start`, `/cctv/control/command`, `/cctv/control/pause`, `/cctv/control/resume`, `/cctv/control/stop` | `POST` | CCTV backend 제어 명령 |
| `/cctv/control/view` | `POST` | 3D 뷰 회전/flip/wire/mesh 제어 |
| `/cctv/control/stream` | `POST` | `pc_stream`, `rgbd_stream`, `depth_stream` 모드 선택 |
| `/cctv/status`, `/cctv/stream` | `GET`, `WS` | CCTV backend 상태 조회 / 바이너리 스트림 수신 |
| `/esp32/watchdog/request`, `/esp32/watchdog/latest`, `/esp32/watchdog/status` | `POST`, `GET` | 상태 재발행 요청 / 최신 payload / MQTT 브리지 상태 조회 |
...
| `/motor/control/press`, `/motor/control/release`, `/motor/control/stop`, `/motor/control/set`, `/motor/control/speed`, `/motor/control/move` | `POST` | 개별 모터 press/release/stop/각도/속도/상대 이동 제어 |
| `/motor/control/stopall`, `/motor/control/center` | `POST` | 전체 모터 정지 / 동일 각도 정렬 |
| `/motor/angles`, `/motor/status` | `GET` | 현재 모터 각도 읽기 / MQTT 브리지 상태 조회 |
| `/motor/emergency` | `POST` | `MOTOR_EMERGENCY_SCRIPT` 또는 기본 비상 대피 모터 시퀀스를 순차 publish |
...
| `/thermal/control/stop` | `POST` | 열화상 WebSocket 종료 요청 및 수신기 정리 시도 |
| `/thermal/status` | `GET` | Thermal UDP 수신 상태 조회 |
| `/thermal/stream` | `WS` | canonical 10B header 기반 열화상 바이너리 스트림 |
...
`POST /motor/emergency` 응답 예시:

```json
{
  "status": "OK",
  "sequence_name": "emergency_evacuation",
  "delivery_mode": "sequential_mqtt_commands",
  "total_steps": 8,
  "attempted_steps": 8,
  "successful_steps": 8,
  "published_steps": 8
...
| 이메일 인증/비밀번호 재설정 API가 실패함 | `crow-mail-secret` 누락, Apps Script URL/secret 오류 | `MAIL_APP_SCRIPT_URL`, `MAIL_SHARED_SECRET`, `MAIL_APP_SCRIPT_TIMEOUT_MS`와 Apps Script 응답 확인 |
| CCTV 제어 실패 | CCTV backend 주소 또는 인증서 문제 | `CCTV_BACKEND_HOST`, `CCTV_BACKEND_PORT`, `/app/certs` 내부 인증서 확인 |
| `/events` 또는 계정 복구 API가 DB 오류를 반환함 | MariaDB에 `event_logs`, `signup_email_verifications`, `password_reset_tokens` 스키마가 없음 | 필요한 테이블 생성 여부와 Crow DB migration 절차를 확인 |
...
- `crow-sunapi-config`, `crow-sunapi-secret`, `crow-cctv-ip-config`, `mariadb-secret`, `crow-admin-secret`, `crow-mail-secret`, `crow-telegram-secret`이 준비되어 있는가
- MariaDB에 `users`, `signup_email_verifications`, `password_reset_tokens`, `event_logs` 스키마가 준비되어 있는가
- `MQTT_HOST`, `MOTOR_RESPONSE_TOPIC`, `LASER_CONTROL_TOPIC`, `MOTOR_EMERGENCY_SCRIPT` 값이 환경과 맞는가
- `JWT_SECRET`, `MAIL_*`, `SUNAPI_STREAMING_WS_PATH`, `SUNAPI_RTSP_HOST` 값이 환경과 맞는가
- `/home/pi/cctv-recordings` 경로가 쓰기 가능한가
- `GET /health`, `GET /docs`, `GET /events`, `GET /recordings`, `GET /laser/status`, `GET /thermal/status`, `GET /esp32/watchdog/status`가 정상 응답하는가
